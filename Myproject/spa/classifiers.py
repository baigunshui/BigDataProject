...
class DictClassifier:
    ...
    def analyse_sentence(self, sentence, runout_filepath=None, print_show=False):
        ...
        if runout_filepath is not None:
            ...
            # if comment_analysis['score'] < -3:
            #     self.__write_runout_file(runout_filepath, "很不喜欢" + "\n\n")
            # elif comment_analysis['score'] < 0:
            #     self.__write_runout_file(runout_filepath, "不喜欢" + "\n\n")
            # elif comment_analysis['score'] == 0:
            #     self.__write_runout_file(runout_filepath, "一般" + "\n\n")
            # elif comment_analysis['score'] <= 3:
            #     self.__write_runout_file(runout_filepath, "喜欢" + "\n\n")
            # elif comment_analysis['score'] > 3:
            #     self.__write_runout_file(runout_filepath, "很喜欢" + "\n\n")
            #self.__write_runout_file(runout_filepath, str(comment_analysis['score']) + "\n\n")
        if print_show:
            print("\n" + sentence)
